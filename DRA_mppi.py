...
"""
DRA_mppi.py (Torch)
- Uses DRA_MPPI from dra_mppi_car.py
- Keeps the SAME loop/plot logic as your CuPy script
- No CuPy import here

Run:
  python3 DRA_mppi.py  --obs-update-steps 20 --scenario 2 --save
"""
...
from dra_mppi_car import DRA_MPPI
...
if __name__ == "__main__":
    ...
    mppi.y_min = torch.as_tensor(y_min_bound, device=mppi.device, dtype=mppi.dtype)
    mppi.y_max = torch.as_tensor(y_max_bound, device=mppi.device, dtype=mppi.dtype)
    mppi.boundary_w = boundary_w
    mppi.boundary_k = boundary_k
    mppi.boundary_d = boundary_d
    mppi._boundary_w_t = torch.as_tensor(boundary_w, device=mppi.device, dtype=mppi.dtype)

    mppi.path_xy = None
    mppi.path_boundary_w = 0.0
    mppi.path_boundary_k = path_boundary_k
    mppi.path_corridor_radius = path_corridor_radius
    ...
    if scenario == 1:
        ...
        mppi.boundary_w = 0.0
        mppi._boundary_w_t = torch.as_tensor(0.0, device=mppi.device, dtype=mppi.dtype)
        ...
        mppi.path_xy = torch.as_tensor(ref_path_xypsi[:, :2], device=mppi.device, dtype=mppi.dtype)
        mppi.path_boundary_w = path_boundary_w
        mppi.path_boundary_k = path_boundary_k
        mppi.path_corridor_radius = path_corridor_radius

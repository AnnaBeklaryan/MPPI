...
- DR-MPPI feasibility filter via CVaR on XYZ distance to moving obstacle:
    g = R_safe - ||(x,y,z) - (ox,oy,oz)||
...
class DRParams:
    ...
    R_u: tuple[float, float, float, float] = (1, 1, 1, 1)
...
class TorchDRMPPIQuadOuter:
    def __init__(self, mass=0.028, g=9.81, params: DRParams | None = None, cylinders=None, device=None):
        ...
        if self.p.sigma is None:
            self.p.sigma = np.array(
                [math.radians(1.3), math.radians(1.3), math.radians(8.0), 0.06*hover],
                dtype=np.float32
            )
    ...
    def plan(
        ...
        if obs_mean_3d_t.ndim == 2:
            O_mean = obs_mean_3d_t[1:self.T+1, :3].unsqueeze(1)  # (T,1,3)
            K = 1
        elif obs_mean_3d_t.ndim == 3:
            O_mean = obs_mean_3d_t[1:self.T+1, :, :3]  # (T,K,3)
...
def simulate(
    ...
    lead_time = 1.5495997771078638  # seconds (aligned with mppi_crazyflie.py)

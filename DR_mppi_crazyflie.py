#!/usr/bin/env python3
"""
Online DR-MPPI Crazyflie outer-loop simulation (Torch) using DR_MPPI from mppi_class.py.
...
LIVE animation:
- Each animation frame does: build ref/obs sequences -> DR_MPPI.plan() -> apply u0 -> update plot
...
def quad_dyn_step(X: torch.Tensor, U: torch.Tensor, dt: float, m: float, g: float, **_kw) -> torch.Tensor:
    # X: (M,7) [px,py,pz,vx,vy,vz,psi], U: (M,4) [T,phi,theta,psidot]
    p = X[:, 0:3]
    v = X[:, 3:6]
    psi = X[:, 6]
...
# ============================================================
# Online simulation (animation drives sim)
# ============================================================
def simulate():
    waypoints = np.array([
        [ 2.5,  2.0, 0.0],
    ...
    ax.set_title("Torch DR-MPPI (online) — Cylinders + Moving Sphere (XY CVaR feasibility)")
    ...
    # logs for drawing the traveled path
    ...
    def update(i: int):
        ...
        solve_text.set_text(f"Solve = {ms:.1f} ms")

        return traj_line, obs_pt, arm1, arm2, center_pt, time_text, solve_text

#!/usr/bin/env python3
"""
Dual-profile autotune for MPPI_final/DR_mppi.py.

Profiles:
- tracking: tunes tracking/controller terms, keeps DR risk params fixed.
- safety: tunes both tracking and DR risk/avoidance terms with stronger collision pressure.
"""
...
# Default fixed risk values from DR_mppi.py
FIXED_RISK = {
    "cvar_alpha": 0.95,
    "cvar_N": 6,
...
def sample_tracking_cfg(rng: random.Random) -> dict:
    return {
        ...
        "u_max_w_deg": rng.uniform(70.0, 170.0),
        "dyn_w_max_deg": rng.uniform(90.0, 190.0),
        "Qx": rng.uniform(0.05, 2.0),
        "Qy": rng.uniform(2.0, 20.0),
        "Qpsi": rng.uniform(0.05, 2.5),
...
def sample_safety_cfg(rng: random.Random) -> dict:
    cfg = sample_tracking_cfg(rng)
    cfg.update(
        {
            "cvar_alpha": rng.uniform(0.90, 0.99),
            "cvar_N": rng.choice([6, 8, 12, 16, 24]),
            "obs_sigma_x": rng.uniform(0.18, 0.45),
            "obs_sigma_y": rng.uniform(0.18, 0.45),
            "dr_eps_cvar": rng.uniform(0.0, 0.06),
            "obs_noise_mode": rng.choice(["static", "per_step"]),
            "extra_margin": rng.uniform(0.03, 0.15),
        }
    )
    return cfg


def _build_controller(cfg: dict, risk: dict, dt: float, device: str):
    T = int(cfg["T"])
    M = int(cfg["M"])
    lam = float(cfg["lam"])
    ...
    mppi = DR_MPPI(
        ...
        obs_noise_mode=str(risk["obs_noise_mode"]),
        dr_eps_cvar=float(risk["dr_eps_cvar"]),
        dyn_kwargs=dict(w_max=dyn_w_max),
        cost_kwargs=dict(ref=None, Q=None, R=None, Qf=None, O_mean=None, radii=None, obs_w=0.0),
        verbose=False,
    )
    return mppi, Q, Qf, R, u_max, dyn_w_max


def evaluate_cfg(cfg: dict, risk: dict, obs_csv: MovingObstacleCSV, pos_scale: float, max_steps: int, device: str, objective: str):
    dt = float(np.round(np.min(np.diff(obs_csv.times)), 6))
    if not (np.isfinite(dt) and dt > 0):
        return 1e12, {"error": "invalid_dt"}

    mppi, Q, Qf, R, u_max, dyn_w_max = _build_controller(cfg, risk, dt, device)
    ...
    extra_margin = float(risk["extra_margin"])
    ...
    for k in range(steps):
        ...
        ids, O_mean, _ = obs_csv.build_prediction_for_mppi(obs_df=obs_now, dt=dt, T=int(cfg["T"]), max_obs=max_obs_draw)
        K = len(ids)
        if K > 0:
    ...
    if objective == "tracking":
        score = 2.0 * lane_rmse + 1.0 * psi_rmse + 0.25 * du_rms + 0.001 * mean_solve_ms
    else:
        # safety-first, while retaining tracking quality
        score = (
            1.8 * lane_rmse
            + 0.9 * psi_rmse
            + 0.2 * du_rms
        ...
        if min_dist < (obs_radius + ego_radius + extra_margin):
            score += 200.0 * ((obs_radius + ego_radius + extra_margin) - min_dist)
    ...
    metrics = {
        ...
        "min_distance": min_dist if np.isfinite(min_dist) else None,
        "steps": steps,
    }
    return float(score), metrics


def run_search(profile: str, trials: int, seed: int, obs_csv: MovingObstacleCSV, pos_scale: float, max_steps: int, device: str):
    ...
    if profile == "tracking":
        tuned_keys = sorted(sample_tracking_cfg(rng).keys())
    else:
        tuned_keys = sorted(sample_safety_cfg(rng).keys())
    ...
    for trial in range(1, trials + 1):
        ...
        score, metrics = evaluate_cfg(
            ...
            max_steps=max_steps,
            device=device,
            objective=("tracking" if profile == "tracking" else "safety"),
        )

        print(
            f"[{profile} {trial:03d}/{trials}] score={score:.4f} "
            f"lane={metrics.get('lane_rmse', float('nan')):.3f} "
            f"psi={metrics.get('psi_rmse', float('nan')):.3f} "
            f"viol={metrics.get('collision_violation_mean', float('nan')):.5f} "
        ...
        if score < best_score:
            ...
            best_risk = risk
            print(f"  -> new best ({profile}) score={best_score:.4f}")
...
def main():
    ap = argparse.ArgumentParser(description="Tune DR_mppi.py for tracking and safety profiles")
    ap.add_argument("--mode", choices=["tracking", "safety", "both"], default="both")
    ap.add_argument("--trials-tracking", type=int, default=1000)
    ap.add_argument("--trials-safety", type=int, default=1000)
    ap.add_argument("--max-steps", type=int, default=300)
    ap.add_argument("--seed", type=int, default=17)
    ap.add_argument("--device", type=str, default="auto", choices=["auto", "cpu", "cuda"])
    ap.add_argument("--csv-path", type=str, default="")
    ap.add_argument("--save-prefix", type=str, default="dr_mppi")
    args = ap.parse_args()
    ...
    if args.mode in ("tracking", "both"):
        outputs.append(
            run_search(
                profile="tracking",
                trials=int(args.trials_tracking),
                seed=int(args.seed),
                obs_csv=obs_csv,
                pos_scale=pos_scale,
    ...
    for out in outputs:
        p = out["profile"]
        save_path = f"{args.save_prefix}_{p}_autotune_best_v2.json"
        with open(save_path, "w", encoding="utf-8") as f:
            json.dump(out, f, indent=2)
        print(f"Saved {p} best to: {save_path}")

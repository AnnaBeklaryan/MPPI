...
"""
DRA_mppi.py (Torch)
- Uses DRA_MPPI from mppi_class.py
- Keeps the SAME loop/plot logic as your CuPy script
- No CuPy import here

Run:
  python3 DRA_mppi.py  --obs-update-steps 10 --scenario 2 --save
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Random seed for DRA-MPPI. If omitted, a fresh random seed is used each run.",
    )
    args = parser.parse_args()

    if args.seed is None:
        sim_seed = secrets.randbits(32)
        seed_label = "random"
    else:
        sim_seed = int(args.seed)
        seed_label = "fixed"
        if sim_seed < 0 or sim_seed >= 2**32:
            raise ValueError(f"--seed must be in [0, {2**32 - 1}], got {args.seed}")

    np.random.seed(sim_seed)
    print(f"[DRA_MPPI] seed={sim_seed} ({seed_label})")
    ...
    # DRA risk settings matched to DRA_mppi.py
    sigma_cp = 0.0
    Nmc = 10
    omega_soft = 100.0
    omega_hard = 10.0
    ...
    mppi = DRA_MPPI(
        ...
        cost_kwargs=dict(
            ref=torch.zeros(3, dtype=torch.float32, device=device),
            Q=torch.as_tensor(Q, dtype=torch.float32, device=device),
            R=torch.as_tensor(R, dtype=torch.float32, device=device),
            Qf=torch.as_tensor(Qf, dtype=torch.float32, device=device),
            O_mean=None,
            radii=None,
            O_phi=None,
            y_min=None,
    ...
    # Put constant boundary/path settings on the DRA controller once.
    mppi.cost_kwargs.update(
        y_min=torch.as_tensor(y_min_bound, device=mppi.device, dtype=mppi.dtype),
        y_max=torch.as_tensor(y_max_bound, device=mppi.device, dtype=mppi.dtype),
    ...
    if scenario == 1:
        ...
        mppi.cost_kwargs.update(boundary_w=0.0)
        ...
        mppi.cost_kwargs.update(
            path_xy=torch.as_tensor(ref_path_xypsi[:, :2], device=mppi.device, dtype=mppi.dtype),
    ...
    for k in range(steps):
        ...
        ids, O_mean, O_phi, draw_pack_ctrl = obs_csv.build_prediction_for_mppi(
            obs_df=obs_ctrl_now, dt=dt, T=T, max_obs=max_obs_draw
        )
        K_used = len(ids)

        if K_used > 0:
            obstacle_buffer_for_mppi = np.full((K_used,), extra_margin, dtype=np.float32)
        else:
            obstacle_buffer_for_mppi = np.array([], dtype=np.float32)
        ...
        mppi.cost_kwargs.update(
            ref=torch.as_tensor(ref, device=mppi.device, dtype=mppi.dtype),
            O_mean=torch.as_tensor(O_mean, device=mppi.device, dtype=mppi.dtype),
            radii=torch.as_tensor(obstacle_buffer_for_mppi, device=mppi.device, dtype=mppi.dtype),
            O_phi=torch.as_tensor(O_phi, device=mppi.device, dtype=mppi.dtype),
            ego_half_length=float(ego_half_length),
            ego_half_width=float(ego_half_width),
            obs_half_length=float(obs_half_length),
            obs_half_width=float(obs_half_width),
            obstacle_buffer=float(extra_margin),
        )
        U, Xsamp = mppi.plan(
            x_mppi,
            return_samples=True,
        ...
        print(
            f"[DRA_MPPI] solve={solve_time*1000:8.2f} ms "

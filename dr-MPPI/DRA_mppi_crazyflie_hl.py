#!/usr/bin/env python3
"""Physical Crazyflie figure-eight flight using DRA-MPPI and Vicon avoidance.
...
def _executed_dra_risk_cost_np(x, obs_now, params, rng):
    obs_points = (
        np.asarray(obs_now, dtype=float).reshape(-1, 3)
    ...
    position = np.asarray(x, dtype=float)[0:3]
    safe_radius = float(
        params.moving_r + params.moving_safety_margin + params.drone_radius
    )
    if safe_radius <= 0.0:
        return 0.0
    sigma = np.maximum(
        np.asarray(params.obs_pos_sigma_xyz, dtype=float).reshape(3),
        1e-6,
    )
    lows = position - safe_radius
    widths = np.full(3, 2.0 * safe_radius, dtype=float)
    samples = lows[None, :] + rng.random(
        (int(params.Nmc), 3)
    ) * widths[None, :]
    cell_volume = float(np.prod(widths)) / max(1, int(params.Nmc))
    norm_const = 1.0 / (
        ((2.0 * np.pi) ** 1.5) * float(np.prod(sigma))
    )
    delta = (
        samples[:, None, :] - obs_points[None, :, :]
    ) / sigma.reshape(1, 1, 3)
    pdf = norm_const * np.exp(-0.5 * np.sum(delta * delta, axis=-1))
    obstacle_mass = np.clip(pdf * cell_volume, 0.0, 0.999)
    joint_mass = 1.0 - np.prod(1.0 - obstacle_mass, axis=1)
    inside = (
        np.sum((samples - position[None, :]) ** 2, axis=1)
        <= safe_radius**2
    )
    phat = float(
        np.clip(np.sum(inside.astype(float) * joint_mass), 0.0, 1.0)
    )
    return (
        float(params.omega_soft) * phat
        + float(params.omega_hard)
        * float(phat > float(params.sigma_cp))
    )
...
def save_physical_flight_log(flight_log, output_dir):
    ...
    data_path = os.path.join(
        output_dir, f"dra_mppi_physical_flight_{timestamp}.npz"
    )
    plot_path = os.path.join(
        output_dir, f"dra_mppi_physical_flight_{timestamp}.png"
    )
    arrays = {
    ...
    figure.suptitle(
        f"Physical DRA-MPPI flight — mean position error "
...
def run_physical_mppi_sequence(
    ...
    try:
        ...
        print(
            f"DRA-MPPI flight started on {ctrl.device}: "
            f"{args.ego_body!r}=DRA-MPPI; "
...
def run_physical_flight(args):
    ...
    print(
        "[DRA_MPPI] "
        f"sigma_cp={params.sigma_cp:.3f}, Nmc={params.Nmc}, "
        f"omega_soft={params.omega_soft:.1f}, "
        f"omega_hard={params.omega_hard:.1f}"
    )
    ctrl = TorchDRAMPPIQuadOuter(
        mass=0.028, g=9.81, params=params, device=device
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Fly physical cf_1 with DRA-MPPI and read cf_2 obstacle data from "

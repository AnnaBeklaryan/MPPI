#!/usr/bin/env python3
"""CrazySim/Gazebo Crazyflie figure-eight flight using DR-MPPI.

Gazebo odometry supplies the controlled vehicle state directly through
``gz topic``; ROS is not required. DR-MPPI solves at high rate, and a farther point
on its optimized prediction is streamed as a position setpoint through cflib.

TERMINAL 1

cd ~/CrazySim/crazyflie-firmware

bash tools/crazyflie-simulation/simulator_files/gazebo/launch/sitl_multiagent_text.sh \
-m crazyflie \
-f two_example.txt

TERMINAL 2

cd /home/user/Downloads/dr-MPPI
python3 -u crazyflie_gazebo_obstacle.py


TERMINAL 3

cd /home/user/Downloads/dr-MPPI
python3 -u mppi_crazyflie_gazebo.py
"""
...
def _executed_cvar_risk_cost_np(x, obs_now, params, rng):
    ...
    cvar_max += float(params.dr_eps_cvar) / max(
        1e-9, 1.0 - float(params.cvar_alpha)
    )
...
class TorchDRMPPIQuadOuter:
    def __init__(self, mass=0.028, g=9.81, params: DRMPPIParams | None = None, device=None):
        self.m = float(mass)
        self.g = float(g)
        self.p = params if params is not None else DRMPPIParams()
...
# -----------------------------
# CrazySim/Gazebo flight
# -----------------------------
@dataclass
class GazeboQuaternion:
    x: float
    y: float
    z: float
    w: float
...
class GazeboTracker(Thread):
    """Read the ego state directly from a Gazebo odometry topic."""
    ...
    def _update(self, name, message, stamp):
        pose = message["pose"]
        ...
        twist_linear = message.get("twist", {}).get("linear", {})
        measured_vel = np.array(
            [
                float(twist_linear.get("x", 0.0)),
                float(twist_linear.get("y", 0.0)),
                float(twist_linear.get("z", 0.0)),
            ],
            dtype=float,
        )
        with self._lock:
    ...
    def wait_for_body(self, name, timeout):
        ...
        while time.monotonic() < deadline:
            if self.error is not None:
                raise RuntimeError("Gazebo odometry reader failed") from self.error
            state = self.snapshot(name)
            if state is not None:
                return state
            time.sleep(0.02)
        raise TimeoutError(
            f"No Gazebo odometry received for {name!r} on {self.ego_topic}"
        )
    ...
    def run(self):
        try:
            self._process = subprocess.Popen(
                [
                    "gz", "topic", "--echo", "--topic", self.ego_topic,
                    "--json-output",
                ],
            ...
            for line in self._process.stdout:
                ...
                try:
                    ...
                    continue
                json_buffer = ""
                self._update(self.ego_name, message, time.monotonic())

            if self._running:
                error_text = self._process.stderr.read().strip()
                raise RuntimeError(
                    error_text or "Gazebo odometry process stopped unexpectedly"
                )
...
def make_obstacle_horizon(tracker, now, horizon_steps, dt, stale_timeout):
    """Predict fresh moving-obstacle odometry at constant velocity."""
...
def save_physical_flight_log(flight_log, output_dir):
    ...
    data_path = os.path.join(output_dir, f"mppi_physical_flight_{timestamp}.npz")
    plot_path = os.path.join(output_dir, f"mppi_physical_flight_{timestamp}.png")
    arrays = {
    ...
    figure.suptitle(
        f"Physical MPPI flight — mean position error "
...
def run_gazebo_flight(args):
    ...
    flight_duration = trajectory_duration + 10.0 * dt
    params = DRMPPIParams(
        dt=dt,
    ...
    print(
        "[DR_MPPI] "
        f"dr_eps_cvar={params.dr_eps_cvar:.6g}, "
    ...
    obstacle_tracker = GazeboTracker(
        args.obstacle_topic,
        ego_name=args.obstacle_body,
        obstacle_name=args.obstacle_body,
    )
    flight_log = {
        "time": [],
    ...
    try:
        print(
            f"Waiting for Gazebo odometry for {args.ego_body!r} "
            f"on {args.ego_topic}..."
        )
        initial_state = tracker.wait_for_body(
            args.ego_body, args.gazebo_timeout
        )
        print(
            "Gazebo odometry acquired at "
            f"({initial_state['position'][0]:.3f}, "
            f"{initial_state['position'][1]:.3f}, "
            f"{initial_state['position'][2]:.3f})"
        )
        print(
            f"Waiting for moving-obstacle odometry for "
            f"{args.obstacle_body!r} on {args.obstacle_topic}..."
        )
        initial_obstacle = obstacle_tracker.wait_for_body(
            args.obstacle_body, args.gazebo_timeout
        )
        print(
            "Moving-obstacle odometry acquired at "
            f"({initial_obstacle['position'][0]:.3f}, "
            f"{initial_obstacle['position'][1]:.3f}, "
            f"{initial_obstacle['position'][2]:.3f})"
        )
        cflib.crtp.init_drivers()
        uri = uri_helper.uri_from_env(default=args.uri)
        with ExitStack() as connections:
            scf = connections.enter_context(
                SyncCrazyflie(uri, cf=Crazyflie(rw_cache="./cache"))
            )
            obstacle_scf = connections.enter_context(
                SyncCrazyflie(
                    args.obstacle_uri,
            ...
            cf = scf.cf
            obstacle_cf = obstacle_scf.cf
            tracker.on_ego_pose = make_extpose_forwarder(
                cf, rate_hz=50.0
            )
            obstacle_tracker.on_ego_pose = make_extpose_forwarder(
                obstacle_cf, rate_hz=50.0
            )
            time.sleep(0.5)
            if tracker.error is not None:
                raise RuntimeError(
                    "Could not forward cf_0 Gazebo poses to the estimator"
                ) from tracker.error
            forwarded_state = tracker.snapshot(args.ego_body)
            if (
                forwarded_state is None
                or time.monotonic() - forwarded_state["stamp"]
                > float(args.ego_timeout)
            ):
                raise RuntimeError(
                    "cf_0 Gazebo odometry stopped before estimator setup"
                )
            forwarded_obstacle = obstacle_tracker.snapshot(
                args.obstacle_body
            )
            if (
                forwarded_obstacle is None
                or time.monotonic() - forwarded_obstacle["stamp"]
                > float(args.obstacle_timeout)
            ):
                raise RuntimeError(
                    "cf_1 Gazebo odometry stopped before estimator setup"
                )
            print("Fresh poses are being forwarded to both estimators.")
            print("Waiting for the cf_0 estimator...")
            configure_estimator(scf)
            print("Waiting for the cf_1 estimator...")
            configure_estimator(obstacle_scf)
            commander = cf.high_level_commander
            obstacle_commander = obstacle_cf.high_level_commander
            obstacle_phase_time = (
                float(args.obstacle_phase) % 1.0
            ) * traj.total_time
            obstacle_initial_ref = traj.eval(obstacle_phase_time)
            obstacle_initial_position = obstacle_initial_ref[0:3]

            print(f"Taking both drones off to {center[2]:.2f} m...")
            try:
                commander.takeoff(
                    float(center[2]), float(args.takeoff_duration)
                )
                obstacle_commander.takeoff(
                    float(center[2]), float(args.takeoff_duration)
                )
                # mocap_hl_commander.py waits 4 s after its 3 s takeoff.
                time.sleep(float(args.takeoff_duration) + 1.0)
                commander.go_to(
                    float(center[0]), float(center[1]), float(center[2]),
                    0.0, float(args.initial_go_to_duration), relative=False,
                )
                obstacle_commander.go_to(
                    float(obstacle_initial_position[0]),
                    float(obstacle_initial_position[1]),
                    float(obstacle_initial_position[2]),
                    float(obstacle_initial_ref[8]),
                ...
                obstacle_commander.land(
                    0.0, float(args.land_duration)
                )
                time.sleep(float(args.land_duration) + 0.5)
                commander.stop()
                obstacle_commander.stop()
                raise

            print(
                f"Synchronized Gazebo flight started on {device}; "
                "cf_0 uses DR-MPPI and cf_1 follows the reverse figure-eight. "
                f"Tracking {int(args.cycles)} figure-eight cycle(s), followed "
                "by 10 final reference steps. "
                "Press Ctrl-C to land both early."
            )
            ...
            try:
                while True:
                    ...
                    obstacle = obstacle_tracker.snapshot(
                        args.obstacle_body
                    )
                    if tracker.error is not None:
                        raise RuntimeError(
                            "Gazebo odometry reader stopped"
                        ) from tracker.error
                    if obstacle_tracker.error is not None:
                        raise RuntimeError(
                            "Moving-obstacle odometry reader stopped"
                        ) from obstacle_tracker.error
                    if ego is None or now - ego["stamp"] > float(args.ego_timeout):
                        raise RuntimeError(
                            f"Lost fresh Gazebo odometry for {args.ego_body!r}"
                        )
                    if (
                        obstacle is None
                        or now - obstacle["stamp"]
                        > float(args.obstacle_timeout)
                    ):
                        raise RuntimeError(
                            f"Lost fresh Gazebo odometry for "
                    ...
                    if elapsed >= flight_duration:
                        print(
                            "Figure-eight complete; landing both drones..."
                        )
                        break

                    obstacle_elapsed = min(elapsed, trajectory_duration)
                    obstacle_time = (
                        obstacle_phase_time - obstacle_elapsed
                    ) % traj.total_time
                    obstacle_ref = traj.eval(obstacle_time)
                    obstacle_cf.commander.send_position_setpoint(
                        float(obstacle_ref[0]),
                        float(obstacle_ref[1]),
                        float(obstacle_ref[2]),
                        math.degrees(float(obstacle_ref[8])),
                    )
                    ...
                    obs_seq = make_obstacle_horizon(
                        ...
                        ctrl.T,
                        dt,
                        args.obstacle_timeout,
                    )
                    obstacle_active = obs_seq is not None
                    if not obstacle_active:
                        raise RuntimeError(
                            "Could not build a fresh cf_1 obstacle horizon"
                        )
                    if obstacle_active != obstacle_was_active:
                        print(
                            f"Active obstacles: {int(obstacle_active)} "
                            f"({args.obstacle_body} {'acquired' if obstacle_active else 'stale/lost'})"
                        )
                        obstacle_was_active = obstacle_active

                    measured_distance = float(
                        np.linalg.norm(x[0:3] - obs_seq[0])
                    )
                    minimum_distance_seen = min(
                        minimum_distance_seen, measured_distance
                    )
                    if (
                        measured_distance <= collision_threshold
                        ...
                        print(
                            "\n"
                            "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n"
                            "COLLISION FLAG: cf_0 and cf_1 collided\n"
                    ...
                    if command_due:
                        _, nominal_xyz = ctrl.plan(
                            x, ref_seq, q, qf, obs_seq,
                            return_predictions=False,
                            return_nominal_prediction=True,
                        )
                        lookahead_index = min(int(args.lookahead_steps), ctrl.T)
                        target = np.asarray(
                    ...
                    if not np.all(np.isfinite(target)):
                        raise RuntimeError(
                            "DR-MPPI produced a non-finite next position"
                        )
                    ...
                    # Position setpoints must be streamed continuously. MPPI
                    # updates the target at command_rate; the latest target is
                    # resent at every control-loop tick.
                    ...
                    flight_log["desired_position"].append(ref_seq[0, 0:3].copy())
                    ...
                    flight_log["obstacle_position"].append(
                        obs_seq[0].copy()
                        if obstacle_active else np.full(3, np.nan, dtype=float)
                    )
                    flight_log["obstacle_active"].append(int(obstacle_active))
                    flight_log["solve_ms"].append(float(solve_ms))
                    if step % 10 == 0:
                        print(
                            f"[step {step:05d}] solve={solve_ms:.1f} ms "
                            f"target_update={int(command_due)} "
                            f"obstacles={int(obstacle_active)} "
                            f"distance={measured_distance:.3f} "
                            f"collision={int(collision_detected)} "
                            f"actual=({x[0]:.3f}, {x[1]:.3f}, {x[2]:.3f}) "
                            f"desired=({ref_seq[0, 0]:.3f}, "
                            f"{ref_seq[0, 1]:.3f}, {ref_seq[0, 2]:.3f}) "
                ...
                print("Landing requested for both drones...")
            finally:
                minimum_text = (
                ...
                # Release the low-level commander before asking the high-level
                # commander to land.
                cf.commander.send_notify_setpoint_stop()
                obstacle_cf.commander.send_notify_setpoint_stop()
                time.sleep(0.1)
                commander.land(0.0, float(args.land_duration))
                obstacle_commander.land(
                    0.0, float(args.land_duration)
        ...
        obstacle_tracker.on_ego_pose = None
        tracker.close()
        obstacle_tracker.close()
        tracker.join(timeout=1.0)
        obstacle_tracker.join(timeout=1.0)
        if not args.no_save:
            try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Fly CrazySim cf_0 with DR-MPPI while cf_1 follows a synchronized "
            "reverse figure-eight obstacle trajectory."
        )
    )
    parser.add_argument("--uri", default="udp://127.0.0.1:19850")
    parser.add_argument(
        "--obstacle-uri",
        default="udp://127.0.0.1:19851",
    )
    parser.add_argument("--ego-topic", default="/cf_0/odom")
    parser.add_argument("--obstacle-topic", default="/cf_1/odom")
    parser.add_argument("--ego-body", default="cf_0")
    parser.add_argument("--obstacle-body", default="cf_1")
    parser.add_argument(
        "--obstacle-phase",
        type=float,
        default=0.25,
        help="cf_1 starting phase as a fraction of one trajectory cycle.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of synchronized figure-eight cycles before landing.",
    )
    parser.set_defaults(
        period=0.02,
        command_rate=20.0,
        ...
        horizon=25,
        rollouts=2000,
        # Gazebo uses a 0.10 x 0.10 m collision box. Its conservative
        # horizontal bounding-circle radius is sqrt(0.05^2 + 0.05^2).
        obstacle_radius=math.hypot(0.05, 0.05),
        drone_radius=math.hypot(0.05, 0.05),
        safety_margin=0.0,
    ...
    parser.add_argument(
        ...
        dest="no_save",
        action="store_false",
        help="Save post-flight data and plots.",
        default=True,
    )
    parser.add_argument(
        "--use-gpu",
    ...
    run_gazebo_flight(parser.parse_args())

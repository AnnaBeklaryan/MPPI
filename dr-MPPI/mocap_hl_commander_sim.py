...
"""
CrazySim/Gazebo version of the high-level commander figure-eight example.

The script reads ground-truth odometry directly from Gazebo's JSON topic
output and forwards it to the simulated Crazyflie estimator through cflib.
ROS and ros_gz_bridge are not required.
"""
import json
import subprocess
import time
...
uri = uri_helper.uri_from_env(
    default='udp://127.0.0.1:19850'
)

# Gazebo publishes the simulated vehicle pose on this bridged ROS 2 topic.
...
class GazeboOdomWrapper(Thread):
    ...
    def run(self):
        command = [
            'gz', 'topic', '--echo', '--topic', self.topic, '--json-output'
        ]
        ...
        print(f'Reading Gazebo pose directly from {self.topic}...')
        json_buffer = ''
        try:
...
if __name__ == '__main__':
    ...
    # Forward Gazebo ground-truth odometry to the simulated Kalman estimator.
    mocap_wrapper = GazeboOdomWrapper(odom_topic)

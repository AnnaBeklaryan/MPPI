...
"""
mppi.py (Torch)
- Uses MPPI from mppi_class.py (Torch-only)
...
from car_geometry import min_signed_distance_to_obstacles_np, rectangle_signed_distance_torch
...
def min_dist_to_obstacles(
    ...
    return min_signed_distance_to_obstacles_np(
        ego_xy=np.asarray(ego_xy, dtype=float),
        ego_phi=float(ego_phi),
        obs_xy=np.asarray(obs_xy, dtype=float),
        obs_phi=np.asarray(obs_phi, dtype=float),
...
def all_obstacles_state(obs_csv, t_query: float) -> tuple[np.ndarray, np.ndarray]:
    obs_df = obs_csv.obstacles_now(t_query)
    if len(obs_df) == 0:
        return np.zeros((0, 2), dtype=np.float32), np.zeros((0,), dtype=np.float32)
    return (
        obs_df[["x", "y"]].to_numpy(dtype=np.float32),
        obs_df["phi"].to_numpy(dtype=np.float32),
    )


def _frame_from_obs_df(obs_df: pd.DataFrame | None) -> int | None:
    if obs_df is None or len(obs_df) == 0 or "frame" not in obs_df.columns:
        return None
    frame_val = obs_df["frame"].iloc[0]
    if pd.isna(frame_val):
        return None
    return int(frame_val)


def _fmt_frame(frame_val: int | None) -> str:
    return f"{int(frame_val):4d}" if frame_val is not None else " n/a"
...
def running_cost_lane_obs(
    ...
    obs_w: float = 0.0,   # plain MPPI can use this obstacle penalty directly
    ...
    **_kwargs,
    ...
    # Optional obstacle penalty used directly by plain MPPI.
    if obs_w != 0.0 and O_mean is not None and O_mean.shape[1] > 0:
        if (
            O_phi is not None
            ...
            sep = rectangle_signed_distance_torch(
                ...
                obs_xy=O_mean[t].unsqueeze(0),
                obs_phi=O_phi[t].unsqueeze(0),
                obs_half_length=obs_half_length,
                obs_half_width=obs_half_width,
            )
            viol = torch.clamp(float(obstacle_buffer) - sep, min=0.0)
        else:
            if radii is None:
                viol = None
            else:
...
def terminal_cost_track(
    ...
    **_kwargs,
    ...
    if terminal_obs_w != 0.0 and O_mean is not None and O_mean.shape[1] > 0:
        t_obs = min(max(int(t_final) - 1, 0), int(O_mean.shape[0]) - 1)
        if (
            O_phi is not None
            and ego_half_length is not None
            and ego_half_width is not None
            ...
            sep = rectangle_signed_distance_torch(
                ...
                ego_half_length=ego_half_length,
                ego_half_width=ego_half_width,
                obs_xy=O_mean[t_obs].unsqueeze(0),
                obs_phi=O_phi[t_obs].unsqueeze(0),
                obs_half_length=obs_half_length,
                obs_half_width=obs_half_width,
            )
            viol = torch.clamp(float(obstacle_buffer) - sep, min=0.0)
            ...
            if radii is None:
                ...
                viol = torch.clamp(radii.unsqueeze(0) - dist, min=0.0)
        if viol is not None:
            cost = cost + float(terminal_obs_w) * torch.sum(viol * viol, dim=1)
...
class MovingObstacleCSV:
    ...
    def predict_horizon_from_state(x0, y0, phi0, v0, a, phi_dot, dt, T):
        ...
        traj_xy = np.zeros((T, 2), dtype=float)
        traj_phi = np.zeros((T,), dtype=float)
        for j in range(T):
            traj_xy[j, 0] = x
            traj_xy[j, 1] = y
            traj_phi[j] = phi
    ...
    def build_prediction_for_mppi(self, obs_df: pd.DataFrame, dt: float, T: int, max_obs: int):
        ...
        O_phi = np.zeros((T, K), dtype=float)
        for k in range(K):
            O_mean[:, k, :], O_phi[:, k] = self.predict_horizon_from_state(
                xs[k], ys[k], phis[k], vs[k], accs[k], phidots[k], dt, T
            )

        draw_pack = dict(ids=ids, xy=np.stack([xs, ys], axis=1), phi=phis)
        return ids, O_mean, O_phi, draw_pack
...
if __name__ == "__main__":
    ...
    # Plain MPPI obstacle-cost settings.
    obs_w = 100
    terminal_obs_w = obs_w
    obs_pos_sigma = np.array([0.07, 0.07], dtype=np.float32)
    ...
    min_solve = float("inf")
    max_solve = 0.0
    safety_threshold = float(extra_margin)
    ...
    for k in range(steps):
        ...
        obs_frame = _frame_from_obs_df(held_obs_df)
        ...
        ids, O_mean, O_phi, draw_pack_ctrl = obs_csv.build_prediction_for_mppi(obs_df=obs_ctrl_now, dt=dt, T=T, max_obs=max_obs_draw)
        K_used = len(ids)
        O_mean_for_mppi = O_mean
        if K_used > 0 and O_mean is not None and O_mean.shape[1] > 0:
            obs_noise = obs_rng.normal(
                loc=0.0,
                scale=obs_pos_sigma.reshape(1, 1, 2),
                size=O_mean.shape,
            ).astype(np.float32)
            O_mean_for_mppi = (O_mean.astype(np.float32) + obs_noise).astype(np.float32)

        if K_used > 0:
            radii_for_mppi = np.full((K_used,), extra_margin, dtype=np.float32)
        else:
        ...
        # ---- update cost kwargs on GPU (plain MPPI reads obstacle state from here) ----
        ...
        if O_mean_for_mppi is not None and O_mean_for_mppi.shape[1] > 0:
            mppi.cost_kwargs["O_mean"] = torch.as_tensor(O_mean_for_mppi, device=mppi.device, dtype=mppi.dtype)
            mppi.cost_kwargs["O_phi"] = torch.as_tensor(O_phi, device=mppi.device, dtype=mppi.dtype)
            mppi.cost_kwargs["radii"] = torch.as_tensor(radii_for_mppi, device=mppi.device, dtype=mppi.dtype)
            mppi.cost_kwargs["ego_half_length"] = float(ego_half_length)
            mppi.cost_kwargs["ego_half_width"] = float(ego_half_width)
            mppi.cost_kwargs["obs_half_length"] = float(obs_half_length)
            mppi.cost_kwargs["obs_half_width"] = float(obs_half_width)
            mppi.cost_kwargs["obstacle_buffer"] = float(extra_margin)
        else:
            mppi.cost_kwargs["O_mean"] = None
            mppi.cost_kwargs["O_phi"] = None
            mppi.cost_kwargs["radii"] = None
            mppi.cost_kwargs["ego_half_length"] = None
            mppi.cost_kwargs["ego_half_width"] = None
            mppi.cost_kwargs["obs_half_length"] = None
            mppi.cost_kwargs["obs_half_width"] = None
        ...
        csv_frame_next = _frame_from_obs_df(obs_csv.obstacles_now(t_next))
        obs_xy_eval, obs_phi_eval = all_obstacles_state(obs_csv, t_next)
        dmin = min_dist_to_obstacles(
            x_mppi[:2],
            x_mppi[2],
            obs_xy_eval,
            obs_phi_eval,
        ...
        print(
            f"[MPPI] solve={solve_time*1000:8.2f} ms "
            f"| frame={k:4d}->{k_next:4d} "
            f"| t={t_now:6.3f}->{t_next:6.3f} "
            f"| csv_next={_fmt_frame(csv_frame_next)} "
            f"| obs_t={held_obs_t:7.3f} "
            f"| obs_f={_fmt_frame(obs_frame)} "
        ...
        plot_ids, _, _, draw_pack_plot = obs_csv.build_prediction_for_mppi(
        ...
        if K_used > 0:
            ...
            obs_used_xy[k, :kk_used, :] = O_mean_for_mppi[0, :kk_used, :2].astype(np.float32)

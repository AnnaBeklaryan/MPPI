...
class RA_MPPI(MPPI):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.include_terminal_risk = True
    ...
    def plan(
        ...
        terminal_cost = self.terminal_cost(X, self.T, **self.cost_kwargs)
        J = torch.nan_to_num(J, nan=torch.tensor(float("inf"), device=self.device, dtype=self.dtype))
        terminal_cost = torch.nan_to_num(
            terminal_cost,
            nan=torch.tensor(float("inf"), device=self.device, dtype=self.dtype),
        )

        def cvar_risk_penalty(cvar_val: Tensor) -> Tensor:
            penalty = torch.zeros_like(cvar_val)
            if self.risk_cost_A > 0.0:
                risk_mask = cvar_val > self.risk_cost_Cu
                penalty[risk_mask] = self.risk_cost_A * cvar_val[risk_mask]
            if penalty.ndim == 2:
                penalty = torch.sum(penalty, dim=0)
            return penalty
        ...
        risk_penalty = cvar_risk_penalty(cvar_max)

        terminal_t = self.T - 1
        O_terminal = None
        if O_mean is not None:
            if O_mean.numel() == 0 or O_mean.shape[1] == 0:
                O_terminal = O_mean
            else:
                obs_idx = min(terminal_t, int(O_mean.shape[0]) - 1)
                O_terminal = O_mean[obs_idx : obs_idx + 1]
        O_phi_terminal = None
        if O_phi is not None:
            phi_idx = min(terminal_t, int(O_phi.shape[0]) - 1)
            O_phi_terminal = O_phi[phi_idx : phi_idx + 1]
        obs_noise_terminal = obs_noise_std
        if obs_noise_std is not None and self.obs_noise_mode == "per_step":
            noise_idx = min(terminal_t, int(obs_noise_std.shape[0]) - 1)
            obs_noise_terminal = obs_noise_std[noise_idx : noise_idx + 1]

        X_terminal_xy = X[:, :pos_dim].unsqueeze(0)
        X_terminal_phi = X[:, 2].unsqueeze(0) if X_hist_phi is not None else None
        _feasible_terminal, cvar_terminal = self.cvar_feasible_mask(
            X_terminal_xy,
            O_terminal,
            radii,
            obs_noise_std=obs_noise_terminal,
            X_hist_phi=X_terminal_phi,
            O_phi=O_phi_terminal,
            ego_half_length=ego_half_length,
            ego_half_width=ego_half_width,
            obs_half_length=obs_half_length,
            obs_half_width=obs_half_width,
        ...
        terminal_risk_penalty = cvar_risk_penalty(cvar_terminal)
        if bool(getattr(self, "include_terminal_risk", True)):
            risk_penalty_total = risk_penalty + terminal_risk_penalty
        else:
            risk_penalty_total = risk_penalty
        J_total = J + terminal_cost + risk_penalty_total

        rho = torch.min(J_total)
        w = torch.exp(-(J_total - rho) / self.lam)
        ...
        debug = {
            ...
            "terminal_cvar_min": float(torch.min(cvar_terminal).item()),
            "terminal_cvar_med": float(torch.median(cvar_terminal).item()),
            "terminal_cvar_max": float(torch.max(cvar_terminal).item()),
            "risk_cost_max": float(torch.max(risk_penalty_total).item()),
            "risk_cost_mean": float(torch.mean(risk_penalty_total).item()),
            "terminal_risk_cost_max": float(torch.max(terminal_risk_penalty).item()),
            "terminal_risk_cost_mean": float(torch.mean(terminal_risk_penalty).item()),
        }
...
class DR_MPPI(RA_MPPI):
    ...
    def cvar_feasible_mask(
        ...
        # DR risk is evaluated step-by-step over obstacle samples, then the
        # planner sums the thresholded per-step violation cost across time.
        Ls, _ = torch.sort(L, dim=-1)                                   # (T,M,K,N)
        cvar_per_obs = self._cvar_from_sorted(Ls)                       # (T,M,K)

        # DR correction (exactly your CuPy)
        if self.dr_eps_cvar > 0.0:
            cvar_per_obs = cvar_per_obs + (self.dr_eps_cvar * self._cvar_lip)

        cvar_time_max = torch.amax(cvar_per_obs, dim=2)                # (T,M)
        feasible = torch.all(cvar_time_max <= 0.0, dim=0)              # (M,)
        return feasible, cvar_time_max

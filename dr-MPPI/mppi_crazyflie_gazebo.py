#!/usr/bin/env python3
"""CrazySim/Gazebo Crazyflie figure-eight flight using MPPI.

Gazebo odometry supplies the controlled vehicle state directly through
``gz topic``; ROS is not required. MPPI solves at high rate, and a farther point
    ...
    **_kwargs
    ...
    if obs_seq is not None:
        ...
        if obs_seq.ndim == 2:
            ...
            signed = dist - (float(moving_r) + float(moving_safety_margin) + float(drone_radius))
...
class TorchMPPIQuadOuter:
    def __init__(self, mass=0.028, g=9.81, params: MPPIParams | None = None, device=None):
        self.m = float(mass)
        self.g = float(g)
...
def simulate(
    ...
    ctrl = TorchMPPIQuadOuter(mass=0.028, g=9.81, params=params, device=device)
...
def run_gazebo_flight(args):
    ...
    params = MPPIParams(
        ...
        w_moving=float(args.obstacle_weight),
    ...
    try:
        ...
        with ExitStack() as connections:
            ...
            print(
                ...
                "cf_0 uses MPPI and cf_1 follows the reverse figure-eight. "
            ...
            try:
                while True:
                    ...
                    if not np.all(np.isfinite(target)):
                        raise RuntimeError("MPPI produced a non-finite next position")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Fly CrazySim cf_0 with MPPI while cf_1 follows a synchronized "

...
MOVING_OBS_COLUMNS = ["epsilon", "moving_obstacle_index", "collision_prob"]
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        help="Output directory for CSV files and plots. Defaults to this script's directory.",
    )
...
def _moving_obstacle_collision_flags(run_data: dict[str, Any]) -> np.ndarray:
    x_path = np.asarray(run_data.get("X_path", []), dtype=float)
    obs_path = np.asarray(run_data.get("obs_path", []), dtype=float)
    radius = float(np.asarray(run_data.get("moving_collision_radius", np.nan), dtype=float))

    if x_path.ndim != 2 or x_path.shape[1] < 3:
        return np.zeros((0,), dtype=bool)
    if obs_path.ndim != 3 or obs_path.shape[0] == 0 or obs_path.shape[2] < 3:
        return np.zeros((0,), dtype=bool)
    if not np.isfinite(radius):
        return np.zeros((obs_path.shape[1],), dtype=bool)

    n = min(int(x_path.shape[0]), int(obs_path.shape[0]))
    if n <= 0:
        return np.zeros((obs_path.shape[1],), dtype=bool)
    diff = obs_path[:n, :, :3] - x_path[:n, None, :3]
    dist = np.linalg.norm(diff, axis=2)
    return np.any(dist < radius, axis=0)


def _summarize_moving_obstacle_collisions(
    eps: float,
    per_run_flags: list[np.ndarray],
) -> list[dict[str, Any]]:
    n_obs = max((int(flags.size) for flags in per_run_flags), default=0)
    rows: list[dict[str, Any]] = []
    for obs_idx in range(n_obs):
        vals = [
            bool(flags[obs_idx])
            for flags in per_run_flags
            if int(flags.size) > obs_idx
        ]
        rows.append(
            dict(
                epsilon=float(eps),
                moving_obstacle_index=int(obs_idx),
                collision_prob=float(np.mean(vals)) if vals else np.nan,
            )
        )
    return rows
...
def _run_one_epsilon_worker(eps: float, run_seeds: tuple[int, ...], args_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    moving_obstacle_collision_flags: list[np.ndarray] = []
    for run_idx, run_seed in enumerate(run_seeds):
        ...
        run_rows.append(row)
        moving_obstacle_collision_flags.append(_moving_obstacle_collision_flags(run_data))
    ...
    return dict(
        ...
        moving_obstacle_rows=_summarize_moving_obstacle_collisions(
            eps=float(eps),
            per_run_flags=moving_obstacle_collision_flags,
        ),
    )
...
def _init_csv_outputs(run_csv: Path, summary_csv: Path, moving_obs_csv: Path) -> None:
    ...
    pd.DataFrame(columns=MOVING_OBS_COLUMNS).to_csv(moving_obs_csv, index=False)
...
def _append_result_to_csv(
    ...
    pd.DataFrame(result["moving_obstacle_rows"], columns=MOVING_OBS_COLUMNS).to_csv(
        moving_obs_csv,
        mode="a",
        header=False,
        index=False,
...
def main() -> None:
    ...
    moving_obs_csv = outdir / "dr_cf_moving_obstacle_collision_probability.csv"
    if bool(args.append):
        if not run_csv.exists():
            pd.DataFrame(columns=RUN_COLUMNS).to_csv(run_csv, index=False)
        if not summary_csv.exists():
            pd.DataFrame(columns=SUMMARY_COLUMNS).to_csv(summary_csv, index=False)
        if not moving_obs_csv.exists():
            pd.DataFrame(columns=MOVING_OBS_COLUMNS).to_csv(moving_obs_csv, index=False)
        print(f"[csv] append=1 keeping existing rows in {outdir}", flush=True)
    else:
        _init_csv_outputs(run_csv, summary_csv, moving_obs_csv)
    ...
    if workers == 1:
        for idx, eps in enumerate(eps_sweep, start=1):
            ...
            _append_result_to_csv(result, run_csv, summary_csv, moving_obs_csv)
    ...
    if bool(args.append):
        ...
        moving_obs_df = _read_csv_if_exists(moving_obs_csv, MOVING_OBS_COLUMNS)
    else:
        run_rows = [row for result in results for row in result["run_rows"]]
        summary_rows = [result["summary_row"] for result in results]
        moving_obs_rows = [row for result in results for row in result["moving_obstacle_rows"]]
        run_df = pd.DataFrame(run_rows, columns=RUN_COLUMNS)
        summary_df = pd.DataFrame(summary_rows, columns=SUMMARY_COLUMNS)
        moving_obs_df = pd.DataFrame(moving_obs_rows, columns=MOVING_OBS_COLUMNS)
    ...
    moving_obs_df = moving_obs_df.sort_values(["epsilon", "moving_obstacle_index"]).reset_index(drop=True)

    run_df.to_csv(run_csv, index=False)
    summary_df.to_csv(summary_csv, index=False)
    moving_obs_df.to_csv(moving_obs_csv, index=False)
    ...
    print(f"[OK] wrote: {moving_obs_csv}", flush=True)

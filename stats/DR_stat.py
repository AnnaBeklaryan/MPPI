...
Example:
python3 stats/DR_stat_parallel.py   --scenario 2   --runs 100   --steps 500   --eps-max 0.1   --eps-step 0.01   --workers 8   --use_gpu
...
) -> tuple[bench.plain_mod.MovingObstacleCSV, np.ndarray, dict, dict, str, dict]:
...
def main() -> None:
    ...
    pd.DataFrame(columns=RUN_CSV_COLUMNS).to_csv(run_csv, index=False)
    pd.DataFrame(columns=SUMMARY_CSV_COLUMNS).to_csv(summary_csv, index=False)
    ...
    for eps in eps_sweep:
        ...
        for run_idx in range(int(args.runs)):
            ...
            append_csv_row(run_csv, run_row, RUN_CSV_COLUMNS)
        ...
        append_csv_row(summary_csv, summary_row, SUMMARY_CSV_COLUMNS)
        print(
